    ...
    if ("geolocation" in navigator) {
      const useUserLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(function (position) {
          ...
          fetch(
            `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=cb38692bd004c473f1b609a257eb5b37`
          )
            .then((e) => e.json())
            .then((data) => {
              ...
              document.querySelector("#speed b").textContent =
                data.wind.speed * 3.6 + "kmph";
              document.querySelector("#humidity b").textContent =
                data.main.humidity;
              document.querySelector("#timezone b").textContent =
                secondsToTimeZoneString(data.timezone);
              document.querySelector("#pressure b").textContent =
                data.main.pressure;
              document.querySelector("#direction b").textContent =
                degreesToWindDirection(data.wind.deg);
              document.querySelector("#uv-index b");
              document.querySelector("#feels b").textContent =
